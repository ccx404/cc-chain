...
mod tests {
    ...
    fn test_memory_storage() {
        ...
        // Test write and read
        assert!(storage.write(contract, key.to_vec(), value.clone()).is_ok());
        let result = storage.read(contract, key).unwrap();
        assert_eq!(result, Some(value));

        // Test exists
        assert!(storage.exists(contract, key).unwrap());
        assert!(!storage.exists(contract, b"nonexistent").unwrap());

        // Test delete
        assert!(storage.delete(contract, key).unwrap());
        assert!(!storage.exists(contract, key).unwrap());
    }
    ...
    fn test_contract_storage() {
        ...
        // Test set and get
        assert!(storage.set(contract, key.clone(), value.clone()).is_ok());
        let result = storage.get(contract, &key).unwrap();
        assert_eq!(result, Some(value));

        // Test cache hit on second read
        let result2 = storage.get(contract, &key).unwrap();
        assert_eq!(result2, Some(b"test_value".to_vec()));

        // Test delete
        assert!(storage.delete(contract, &key).unwrap());
        let result3 = storage.get(contract, &key).unwrap();
    ...
    fn test_storage_root() {
        ...
        // Add some data
        storage
            .set(contract, b"key1".to_vec(), b"value1".to_vec())
            .unwrap();
        storage
            .set(contract, b"key2".to_vec(), b"value2".to_vec())
            .unwrap();

        let root1 = storage.storage_root(contract).unwrap();

        // Add more data
        storage
            .set(contract, b"key3".to_vec(), b"value3".to_vec())
            .unwrap();

        let root2 = storage.storage_root(contract).unwrap();
    ...
    fn test_cache_management() {
        ...
        // Initially no cache
        let (contracts, entries) = storage.cache_stats();
        assert_eq!(contracts, 0);
        assert_eq!(entries, 0);

        // After read, should have cache
        let _ = storage.get(contract, b"key");
        storage.clear_cache(contract);

        let (contracts2, entries2) = storage.cache_stats();
        assert_eq!(contracts2, 0);
        assert_eq!(entries2, 0);
    }
}
