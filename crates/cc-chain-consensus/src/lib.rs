...
//!
//! This crate provides enhanced BFT consensus protocol for CC Chain with:
...
use cc_chain_sdk::{Block, CCError, Result, CCKeypair, CCPublicKey, CCSignature, Hash};
...
pub struct CCConsensus {
    ...
    safety_system: std::sync::Arc<safety::SafetySystem>,
...
impl CCConsensus {
    ...
    pub fn new(keypair: CCKeypair) -> Self {
        ...
        let safety_config = safety::SafetyConfig::default();
        let safety_system = std::sync::Arc::new(safety::SafetySystem::new(safety_config));
    ...
    pub fn new_with_safety_config(keypair: CCKeypair, safety_config: safety::SafetyConfig) -> Self {
        let height = 0;
        let round = 0;
        ...
        if !proposer.verify(&proposal_data, &signature) {
            ...
            if self.params.safety_monitoring_enabled {
                let _ = self.safety_system.monitor_validator_behavior(
                    proposer,
                    safety::ValidatorAction::ProposalMade {
    ...
    fn handle_vote(
        ...
        if !voter.verify(&vote_data, &signature) {
            ...
            if self.params.safety_monitoring_enabled {
                let _ = self.safety_system.monitor_validator_behavior(
                    voter,
                    safety::ValidatorAction::VoteCast {
                        block_hash,
                        consistent: false,
                    },
                );
    ...
    pub fn get_safety_status(&self) -> safety::SafetyStatus {
