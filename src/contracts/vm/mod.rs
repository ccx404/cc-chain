...
mod tests {
    ...
    fn test_vm_creation() {
        ...
        let vm = vm.unwrap();
        assert_eq!(vm.remaining_gas(), 10_000_000);
        assert_eq!(vm.get_event_count(None), 0);
    }
    ...
    fn test_vm_config_defaults() {
        ...
        assert_eq!(config.deployment_gas_cost, 50_000);
    ...
    fn test_event_system_integration() {
        let config = VMConfig::default();
        let mut vm = SmartContractVM::new(config).unwrap();

        // Test event emission
        let result = vm.emit_event(
        ...
        assert!(result.is_ok());
        assert_eq!(vm.get_event_count(Some("contract123")), 1);
    }

    #[test]
    fn test_inter_contract_call_integration() {
        let config = VMConfig::default();
        let mut vm = SmartContractVM::new(config).unwrap();
        ...
        // This would fail because target contract doesn't exist, but shouldn't panic
        let result = vm.inter_contract_call(call, "caller".to_string(), "origin".to_string());

        // Should return a result indicating failure, not an error
        assert!(result.is_ok());
